...
async function importExcelData(userId, fileBuffer) {
  ...
  const session = await ExpanseDetail.startSession();
    ...
    for (let i = 0; i < jsonData.length; i++) {
      ...
      if (!category) {
        category = new Category({
          user_id: userId,
          category_name: categoryName,
          category_type: "expense", // Adjust as needed
          description: null,
        });
      ...
      bulkInsertData.push(newExpanseDetail);
    }

    // Insert all data in bulk
    await ExpanseDetail.insertMany(bulkInsertData, { session });
